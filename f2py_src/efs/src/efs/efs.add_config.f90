SUBROUTINE add_config(rcut_in, alat_in, uv_in, labels_in, coords_in, energy_in, forces_in, stresses_in, f_on, s_on)
!###########################################################
...
REAL(kind=DoubleReal) :: energy_in
REAL(kind=DoubleReal) :: forces_in(:,:)
REAL(kind=DoubleReal) :: stresses_in(1:3,1:3)
INTEGER(kind=StandardInteger) :: f_on
INTEGER(kind=StandardInteger) :: s_on
